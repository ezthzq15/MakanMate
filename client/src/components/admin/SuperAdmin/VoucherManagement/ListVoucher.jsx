...
import { 
  ...
  IconFileTypeCsv, IconTicket, IconSparkles
} from '@tabler/icons-react';
import { useTableVouchers } from '../../../../hooks/admin/SuperAdmin/VoucherManagement/useTableVouchers';
import AddVoucher from './AddVoucher';
import EditVoucher from './EditVoucher';

const ListVoucher = () => {
  const { vouchers, loading, fetchVouchers, deleteVoucher, generateRandomVoucher } = useTableVouchers();
  ...
  const filtered = vouchers.filter((v) => {
    ...
    const stallMatch = (v.stallName || '').toLowerCase().includes(search.toLowerCase());
    const matchesSearch = titleMatch || stallMatch;
  ...
  const exportCSV = () => {
    const headers = ['Title', 'Stall', 'Discount', 'Min Spend', 'Valid Until', 'Quantity', 'Redeemed', 'Status'];
    const rows = filtered.map((v) => {
      const discount = v.discountType === 'Percentage' ? `${v.discountValue}%` : (v.discountType === 'Fixed Amount' ? `RM ${v.discountValue}` : v.discount);
      return [
        v.title, v.stallName || 'Unknown Stall', discount, v.minSpend ? `RM ${v.minSpend}` : '-',
    ...
    a.href = url;
    a.download = `admin_vouchers_export_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <Paper p="xl" withBorder radius="md">
      ...
      <Group justify="space-between" mb="xl" wrap="wrap" align="center">
        ...
        <Group gap="sm" wrap="wrap">
          <TextInput
            placeholder="Search by title or stall..."
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((voucher) => {
                  ...
                  <Table.Tr key={voucher.id}>
                    ...
                    <Table.Td>{voucher.stallName || 'Unknown Stall'}</Table.Td>
                    <Table.Td>
